            ...
            switch (_b.label) {
                ...
                case 1:
                    ...
                    maps = mappers_1.toArray($('.mapholder')).map(function (mapEl) {
                        var result = mapEl.find('.results span');
                        var curSecondHalf = result.find('.t').length === 1;
                        var t_first = result.find('.t').first().text();
                        var ct_first = result.find('.ct').first().text();
                        var t_second = curSecondHalf ? 0 : result.find('.t').last().text();
                        var ct_second = curSecondHalf ? 0 : result.find('.ct').last().text();
                        return {
